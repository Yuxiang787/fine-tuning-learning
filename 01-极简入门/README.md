...
1. 加载数据 → 2. 加载模型 → 3. 配置 LoRA → 4. 训练 → 5. 保存
     ↓              ↓             ↓            ↓         ↓
  data.jsonl   Qwen2.5-0.5B    r=16, alpha=32   3 epochs  adapter/
```
...
```python
# 推荐教学模型（按大小排序）
MODEL_NAME = "Qwen/Qwen2.5-0.5B"       # 0.5B，中文友好（默认推荐）
MODEL_NAME = "facebook/opt-125m"       # 125M，流程测试更快
...
```python
BATCH_SIZE = 8          # M4 24GB 推荐，显存允许情况下可继续调大
